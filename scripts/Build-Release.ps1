...
# get xml for kxnprod
~/bin/OpenKNXproducer.exe create --Debug --Output=release/SensorModule.knxprod --HeaderFileName=src/Sensormodul.h src/Sensormodul-Release.xml
if (!$?) {
    Write-Host "Error in knxprod, Release was not built!"
    exit 1
}
Move-Item src/Sensormodul-Release.debug.xml release/data/SensorModule.xml
...
# cleanup
Remove-Item release/SensorModule.knxprod

# create package 
Compress-Archive -Path release/* -DestinationPath Release.zip
Remove-Item -Recurse release/*
Move-Item Release.zip release/SensorModule.zip
